...
- **重试预算**：默认 `max_retries=8`，即**最多尝试 9 次**（首次请求 + 8 次重试）。
- **退避策略**：对需要重试的情况使用指数退避（基于 `retry_wait_seconds=1.0`），延迟序列为：
  `1, 2, 4, 8, 16, 32, 32, 32...`（最大 32 秒封顶）。
- **会重试的情况**：
  - **HTTP 429** 或 **HTTP 5xx**
  - **网络异常**（如上游连接失败/超时等）
- **不会重试的情况**：
  - **HTTP 4xx（除 429）**：认为是客户端请求问题，直接返回。

不同模式下的行为：

- **有 key 模式**：遇到 429/5xx/网络异常会标记当前 key 失败（必要时进入冷却），并在预算内切换其它 key 继续尝试。
- **无 key（free tier）模式**：仅对 429/5xx/网络异常进行指数退避重试；预算耗尽后返回最后一次响应（或抛出最后一次异常）。

另外：

- **客户端断开连接**：如果客户端在重试/退避期间断开连接，服务会中止执行并返回 **HTTP 499**（Client Closed Request）。
